...
int WiFiServer2::setTimeout(uint32_t seconds){
  struct timeval tv;
  tv.tv_sec = seconds;
  tv.tv_usec = 0;
...
size_t WiFiServer2::write(const uint8_t *data, size_t len){
  return 0;
}

void WiFiServer2::stopAll(){}

WiFiClient WiFiServer2::available(){
  if(!_listening)
    return WiFiClient();
  int client_sock;
...
void WiFiServer2::begin(uint16_t port){
  if(_listening)
    return;
  if(port){
      _port = port;
  ...
  int val = 1;
    if(int i = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (char*)&val, sizeof(int))  <0){
      printf("socket: %d %s\n", i, strerror(i));
    }
...
void WiFiServer2::setNoDelay(bool nodelay) {
    _noDelay = nodelay;
}

bool WiFiServer2::getNoDelay() {
    return _noDelay;
}

bool WiFiServer2::hasClient() {
    if (_accepted_sockfd >= 0) {
      return true;
    }
    struct sockaddr_in _client;
...
void WiFiServer2::close(){
  end();
}

void WiFiServer2::stop(){
  end();
}
